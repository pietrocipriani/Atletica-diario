...
class Athlete with auth.Notifier<Athlete> {
  ...
  static bool exists(final Document ref) =>
      Globals.coach.athletes.contains(ref);
  ...
  bool get isAthlete =>
      group != null &&
      (Globals.coach.fictionalAthletes || athlete != null) &&
      (Globals.coach.showAsAthlete || athlete != Globals.coach.userReference);

  bool dismissed = false;

  factory Athlete.parse(final DocumentSnapshot raw, final bool exists) {
    final Athlete p = Athlete._parse(raw, exists);
  ...
  Athlete._parse(DocumentSnapshot raw, bool exists)
    ...
    // TODO: check if the use of the setter is unwanted
    group = cast<String?>(raw.getNullable('group'), null);
    _createTrainingsCountSubscription();
    sortedFullAthletes.add(this);
  }
  factory Athlete.update(final Athlete p, final DocumentSnapshot raw) {
  ...
  Stream<QuerySnapshot> _resultSnapshots() {
    ...
    return ref
        .collection('results')
        .where('coach', isEqualTo: Globals.coach.uid)
        .snapshots();
  }*/
  ...
  static void remove(final Athlete p) {
    sortedFullAthletes.remove(p);
    _cache.notifyAll(p, auth.Change.DELETED);
  }
  ...
  static Future<void> create({
    ...
    required String group,
      ...
      Globals.coach.addAthlete(
        null,
        nickname,
        group,
      );

  /// deletes `this` Athlete from `firestore`.
  Future<void> delete() {
    dismissed = true;
    _trainingsCountSubscription.cancel();
    final WriteBatch batch = firestore.batch();
    batch.delete(reference);
    if (athlete == Globals.helper.userReference)
      batch.update(Globals.helper.userReference, {
